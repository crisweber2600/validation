...
public class ReliableDeleteValidationConsumer<T> : IConsumer<DeleteRequested<T>>
{
    ...
    public async Task Consume(ConsumeContext<DeleteRequested<T>> context)
    {
        ...
        catch (DeletePipelineCircuitOpenException ex)
        {
            ...
            await context.Publish(new DeleteValidationFailed<T>(context.Message.AppName, context.Message.EntityType, context.Message.EntityId, "Circuit breaker open"),
                context.CancellationToken);
        ...
        catch (DeletePipelineReliabilityException ex)
        {
            ...
            await context.Publish(new DeleteValidationFailed<T>(context.Message.AppName, context.Message.EntityType, context.Message.EntityId, ex.Message),
                context.CancellationToken);
