...
public class SaveValidationConsumer<T> : IConsumer<SaveRequested<T>>
{
    ...
    public async Task Consume(ConsumeContext<SaveRequested<T>> context)
    {
        ...
        await context.Publish(new SaveCommitRequested<T>(context.Message.AppName, context.Message.EntityType, context.Message.EntityId, context.Message.Payload, audit.Id));
    }
}